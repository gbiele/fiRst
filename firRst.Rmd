...
```{r, eval=FALSE}
install.packages(c("ggplot2", "cowplots"
                  "psych",
                  "sjplot", "ggeffect",
                  "VIM",
                  "VGAM")
...
Our _exposure_ are factor scores (mean = 0, sd = 1) from a factor analysis of MoBa questions about parenting style at age 5. Our _outcome_ is a sum-score for ADHD symptoms from the questionnaire at age 8. We also adjust for gender.
...
This looks kind of complicated. Lets make a table in APA format from this:
...
Note that the results were saved in a word file named reg_table_APA.doc!.
...
In this document we have mixed data preparation and data analysis. Typically, I would have one or several script for data preparation, and one for data analysis. In this spirit, lets save the data so that we can load them another time for further analysis.
```{r}
save(my_data,file = "data/mysdata.Rdata")
```
